"""
EDA Grafana controller — entry point.
...
Long-running controller that, every RECONCILE_INTERVAL:
  ...
  * pushes one dashboard per namespace (uid topo-<ns>, tag 'grafana') to the
    bundled Grafana via its HTTP API (admin basic-auth),
  * (re)creates the read-only 'grafana-menu' home dashboard that lists them.
...
GRAFANA_ROOTURL_CM = "grafana-rooturl"
...
GRAFANA_LABEL = "app=grafana,eda.nokia.com/app-group=grafana"
APP_LABELS = {"eda.nokia.com/app": "eda-grafana"}
...
ROOTURL_ANNO = "grafana.eda.edacommunity.com/restarted-for"

CRD_GROUP = "grafana.eda.edacommunity.com"
CRD_VERSION = "v1alpha1"
CRD_PLURAL = "grafanaconfigs"
CRD_KIND = "GrafanaConfig"
CRD_NAME = "default"

# never draw dashboards for infrastructure namespaces
SYSTEM_NS = {"eda-system", "eda-grafana", "kube-system"}
...
def reconcile(cfg):
    ...
    try:
        if not grafana.dashboard_exists(GRAFANA_URL, menu_uid, GRAFANA_USER, GRAFANA_PASSWORD):
            grafana.push_dashboard(GRAFANA_URL, menu, GRAFANA_USER, GRAFANA_PASSWORD, "grafana menu")
...
def main():
    _setup_logging()
    logger.info("Grafana controller started (version %s); grafana=%s interval=%ds",
