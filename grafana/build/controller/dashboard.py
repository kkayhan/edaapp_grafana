...
Assemble Grafana dashboards.
  ...
  build_topology_dashboard: wrap the flow-panel reference (assets/flowpanel.ref.json)
    ...
    the target namespace. One dashboard per fabric namespace, tag 'grafana'.
  build_menu_dashboard: the read-only landing/home dashboard -- a 'dashlist' panel
    that auto-lists every grafana-tagged dashboard, so the user picks a namespace.

EQL-direct data path: the flow panel's single target is an Infinity (JSON/URL)
query against the Grafana controller's own /eql/<ns>.json endpoint, which serves
...
DATA_URL = os.environ.get(
    "GRAFANA_DATA_URL",
    "http://eda-grafana.eda-system.svc.cluster.local:8080/eql/${namespace}.json")
...
def build_menu_dashboard(uid="grafana-menu", title="Grafana — fabric dashboards"):
    header = {
