...
Two hardening measures (both learned from edaapp_UserAudit, verified on the customer):
  ...
  2. EDA API token via a self-provisioned SERVICE ACCOUNT (client_credentials), not a
     ...
     dedicated confidential client `eda-topoview`, grant its service account the
...
def _ensure_service_client(admin_token):
    """Idempotently ensure the dedicated `eda-topoview` confidential service-account
    ...
    if not kc_id:
        body = {
            ...
            "name": "EDA TopoView (service account)",
            "description": ("Dedicated service-account client for the EDA TopoView app. "
                            "Auth via client_credentials; no human password. Self-managed by "
                            "the eda-topoview controller — safe to delete to revoke access."),
            "enabled": True,
